...
def get_data():
    ...
    angle = file_open['angle'].value
    excited_b = file_open['excited_b'].value
    ground_b = file_open['ground_b'].value
...
def t1_fit(i_data, q_data, xdata):
    angle, excited_b, ground_b = get_data()
    iq_new = rotate_complex(i_data, q_data, angle)
    fit(xdata, iq_new.real, iq_new.imag, plot=True, method='t1')
    hline(excited_b, ground_b)
    return

def t2_ramsey_fit(i_data, q_data, xdata):
    angle, excited_b, ground_b = get_data()
    iq_new = rotate_complex(i_data, q_data, angle)
    f_detune, out = fit(xdata, iq_new.real, iq_new.imag,
                        plot=True, method='t2R')
    print 'The qubit drive frequency has been detuned', f_detune, ' MHz'
    hline(excited_b, ground_b)
    return

def t2_echo_fit(i_data, q_data, xdata):
    angle, excited_b, ground_b = get_data()
    iq_new = rotate_complex(i_data, q_data, angle)
    fit(xdata, iq_new.real, iq_new.imag, plot=True, method='t2E')
    hline(excited_b, ground_b)
    return

def allxy(i_data, q_data, xdata):
    angle, excited_b, ground_b = get_data()
    iq_new = rotate_complex(i_data, q_data, angle)
    plt.figure()
    plt.plot(iq_new.real, 'o-')
    hline(excited_b, ground_b)
    return